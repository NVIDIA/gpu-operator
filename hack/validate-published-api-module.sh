...
VERSION_PACKAGE="github.com/NVIDIA/gpu-operator/internal/info"
...
if [[ -z "${OPERATOR_VERSION:-}" ]]; then
    printf 'ERROR: OPERATOR_VERSION is not set\n' >&2
    exit 1
fi

if [[ "${OPERATOR_VERSION}" != "${OPERATOR_TAG}" ]]; then
    printf 'ERROR: versions.mk declares %s, but the operator tag is %s\n' \
        "${OPERATOR_VERSION}" "${OPERATOR_TAG}" >&2
    exit 1
fi
...
CGO_ENABLED=0 GOOS=linux make --no-print-directory cmd-gpu-operator \
    VERSION="${OPERATOR_VERSION}" \
    GIT_COMMIT="${operator_commit}" \
    COMMAND_BUILD_OPTIONS="-o ${binary}"
...
if ! grep -Fq -- "-X ${VERSION_PACKAGE}.version=${OPERATOR_VERSION}" <<< "${metadata}"; then
    printf 'ERROR: operator binary does not record operator version %s\n%s\n' \
        "${OPERATOR_VERSION}" "${metadata}" >&2
    exit 1
fi
