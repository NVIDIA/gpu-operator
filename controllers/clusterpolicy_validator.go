...
func validateDRA(clusterpolicy *gpuv1.ClusterPolicy, draSupported bool) error {
	if !draSupported && clusterpolicy.Spec.DRADriver.IsEnabled() {
		return fmt.Errorf("the NVIDIA DRA driver is enabled in ClusterPolicy but Dynamic Resource Allocation is not enabled in the kubernetes cluster")
	}

	if clusterpolicy.Spec.DevicePlugin.IsEnabled() && clusterpolicy.Spec.DRADriver.IsGPUsEnabled() {
		return fmt.Errorf("the device-plugin and DRA driver for GPUs cannot both be enabled in ClusterPolicy")
	}
