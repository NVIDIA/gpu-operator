...
func TestDeferDRAPluginRemoval(t *testing.T) {
	...
	t.Run("claim pod on node defers plugin label removal", func(t *testing.T) {
		nlc := &nodeLabelingController{
			client:        fake.NewClientBuilder().WithScheme(scheme).WithObjects(gpuClaim.DeepCopy(), claimPod.DeepCopy()).Build(),
			clusterPolicy: &gpuv1.ClusterPolicy{},
			gpuCluster:    &nvidiav1alpha1.GPUCluster{},
			logger:        logr.Discard(),
		}
	...
	t.Run("admin-access claim pod defers plugin label removal", func(t *testing.T) {
		...
		nlc := &nodeLabelingController{
			client:        fake.NewClientBuilder().WithScheme(scheme).WithObjects(adminClaim, adminPod).Build(),
			clusterPolicy: &gpuv1.ClusterPolicy{},
			gpuCluster:    &nvidiav1alpha1.GPUCluster{},
			logger:        logr.Discard(),
		}
	...
	t.Run("terminating claim pod still defers", func(t *testing.T) {
		...
		nlc := &nodeLabelingController{
			client:        fake.NewClientBuilder().WithScheme(scheme).WithObjects(gpuClaim.DeepCopy(), terminating).Build(),
			clusterPolicy: &gpuv1.ClusterPolicy{},
			gpuCluster:    &nvidiav1alpha1.GPUCluster{},
			logger:        logr.Discard(),
		}
