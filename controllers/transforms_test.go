...
func TestDriverConfigDigest(t *testing.T) {
	baseDigest := utils.GetObjectHashIgnoreEmptyKeys(extractDriverInstallConfig(baseDriverDaemonSetSpec()))
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.modify(spec)
			digest := utils.GetObjectHashIgnoreEmptyKeys(extractDriverInstallConfig(spec))
			if tc.wantChange {
...
func TestExtractDriverInstallConfigExtraction(t *testing.T) {
	...
	cfg := extractDriverInstallConfig(spec)
