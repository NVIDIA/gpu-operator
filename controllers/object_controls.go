...
func createSecretVolumeMounts(n ClusterPolicyController, secretName string, destinationDir string) ([]corev1.VolumeMount, []corev1.KeyToPath, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("ERROR: could not get Secret %s from client: %w", secretName, err)
	}
...
func applyLicensingConfig(obj *appsv1.DaemonSet, config *gpuv1.ClusterPolicySpec, driverIndex int) {
	if config.Driver.LicensingConfig == nil || (config.Driver.LicensingConfig.SecretName == "" && config.Driver.LicensingConfig.ConfigMapName == "") {
		return
	}
	podSpec := &obj.Spec.Template.Spec
...
func transformDriverContainer(obj *appsv1.DaemonSet, config *gpuv1.ClusterPolicySpec, n ClusterPolicyController) error {
	...
	applyLicensingConfig(obj, config, driverIndex)
