...
func TransformDRADriverKubeletPlugin(obj *appsv1.DaemonSet, config *gpuv1.ClusterPolicySpec, n ClusterPolicyController) error {
	...
	for i, container := range obj.Spec.Template.Spec.Containers {
		...
		if container.Name == "gpus" {
			...
			for _, env := range config.DRADriver.KubeletPlugin.GPUs.Env {
				setContainerEnv(&(obj.Spec.Template.Spec.Containers[i]), env.Name, env.Value)
			}

			obj.Spec.Template.Spec.Containers[i].Resources.Requests = config.DRADriver.KubeletPlugin.GPUs.Resources.Requests
			obj.Spec.Template.Spec.Containers[i].Resources.Limits = config.DRADriver.KubeletPlugin.GPUs.Resources.Limits
		}

		// update the "compute-domains" container
		if container.Name == "compute-domains" {
			for _, env := range config.DRADriver.KubeletPlugin.ComputeDomains.Env {
				setContainerEnv(&(obj.Spec.Template.Spec.Containers[i]), env.Name, env.Value)
			}

			obj.Spec.Template.Spec.Containers[i].Resources.Requests = config.DRADriver.KubeletPlugin.ComputeDomains.Resources.Requests
			obj.Spec.Template.Spec.Containers[i].Resources.Limits = config.DRADriver.KubeletPlugin.ComputeDomains.Resources.Limits
		}
	}
...
func TransformDRADriverController(obj *appsv1.Deployment, spec *gpuv1.ClusterPolicySpec) error {
	...
	// set tolerations if specified
	if len(config.Controller.Tolerations) > 0 {
		obj.Spec.Template.Spec.Tolerations = append(obj.Spec.Template.Spec.Tolerations, config.Controller.Tolerations...)
	}

	// set envvars if specified
	for _, env := range config.Controller.ComputeDomains.Env {
		setContainerEnv(&(obj.Spec.Template.Spec.Containers[0]), env.Name, env.Value)
	}

	// update resource requests/limits
	obj.Spec.Template.Spec.Containers[0].Resources.Requests = config.Controller.ComputeDomains.Resources.Requests
	obj.Spec.Template.Spec.Containers[0].Resources.Limits = config.Controller.ComputeDomains.Resources.Limits
...
func createDeviceClass(n ClusterPolicyController, spec resourceapi.DeviceClass) (gpuv1.State, error) {
	...
	// Check if state is disabled and cleanup resource if exists
	// TODO: selectively create DeviceClasses based on what resources are enabled in ClusterPolicy
	if !n.isStateEnabled(n.stateNames[state]) {
		err := n.client.Delete(ctx, obj)
		if err != nil && !apierrors.IsNotFound(err) {
			logger.Info("Couldn't delete", "Error", err)
