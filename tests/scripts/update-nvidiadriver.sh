...
get_default_nvidiadriver_name() {
    kubectl get nvidiadriver -o json --request-timeout="${KUBECTL_REQUEST_TIMEOUT}" |
...
wait_for_nvidiadriver_owner() {
    ...
    while :; do
        ...
        echo "NVIDIADriver/${driver_name} owns ${owned_count:-unknown}/${gpu_node_count:-unknown} GPU node(s)"
        sleep 5
    done
}
...
get_nvidiadriver_daemonsets() {
    local driver_name=$1
    kubectl get daemonset -l "app.kubernetes.io/component=nvidia-driver" -n "$TEST_NAMESPACE" -o json --request-timeout="${KUBECTL_REQUEST_TIMEOUT}" |
...
wait_for_nvidiadriver_daemonsets() {
    ...
    while :; do
        ...
        if (( SECONDS > deadline )); then
            ...
            kubectl get daemonset -l "app.kubernetes.io/component=nvidia-driver" -n "$TEST_NAMESPACE" -o yaml --request-timeout="${KUBECTL_LOG_TIMEOUT}" || true
            exit 1
        fi
...
test_driver_image_updates() {
    ...
    while :; do
        ...
        if (( SECONDS > deadline )); then
            ...
            get_nvidiadriver_daemonsets "${NVIDIA_DRIVER_NAME}" || true
            exit 1
        fi
...
test_custom_labels_override() {
  ...
  if ! labeled_pod_count=$(kubectl_count get pods -n "$TEST_NAMESPACE" -l "app.kubernetes.io/component=nvidia-driver,cloudprovider=aws,platform=kubernetes" --no-headers); then
    echo "cannot count the labelled NVIDIADriver/${NVIDIA_DRIVER_NAME} pods"
    exit 1
  fi
  if ! gpu_node_count=$(kubectl_count get node -l nvidia.com/gpu.present=true --no-headers); then
    echo "cannot count the GPU nodes"
    exit 1
  fi
  if [[ "${gpu_node_count}" -eq 0 ]]; then
    echo "no GPU nodes found while verifying NVIDIADriver/${NVIDIA_DRIVER_NAME} labels"
    exit 1
  fi
  if [[ "${labeled_pod_count}" -ne "${gpu_node_count}" ]]; then
    ...
    kubectl get pods -n "$TEST_NAMESPACE" -l "app.kubernetes.io/component=nvidia-driver" --show-labels --request-timeout="${KUBECTL_REQUEST_TIMEOUT}" || true
    exit 1
  fi
}
...
assert_nvidiadriver_owner_count() {
    ...
    if ! owned_count=$(kubectl_count get nodes -l "nvidia.com/gpu.present=true,nvidia.com/gpu-operator.driver.owner=${driver_name}" --no-headers); then
        echo "cannot count the nodes owned by NVIDIADriver/${driver_name}"
        exit 1
    fi
    if [[ "${gpu_node_count}" -eq 0 ]]; then
        echo "no GPU nodes found while checking NVIDIADriver/${driver_name} ownership"
        exit 1
    fi
...
wait_for_nvidiadriver_condition_message() {
    ...
    while :; do
        if kubectl get nvidiadriver/"${driver_name}" -o json --request-timeout="${KUBECTL_REQUEST_TIMEOUT}" | jq -e --arg message "${message}" '
        ...
        if (( SECONDS > deadline )); then
            ...
            kubectl get nvidiadriver/"${driver_name}" -o yaml --request-timeout="${KUBECTL_LOG_TIMEOUT}" || true
            exit 1
        fi
...
wait_for_nvidiadriver_ready() {
    ...
    while :; do
        if kubectl get nvidiadriver/"${driver_name}" -o json --request-timeout="${KUBECTL_REQUEST_TIMEOUT}" | jq -e '
