...
// CreateNamespace creates a kubernetes namespace based on the "name" parameter provided in the function args
func CreateNamespace(ctx context.Context, name string, c clientset.Interface, labels map[string]string) (*corev1.Namespace, error) {
	if labels == nil {
		labels = map[string]string{}
	}
	labels["e2e-run"] = string(RunID)
	...
	if err := wait.PollUntilContextTimeout(ctx, PollInterval, 30*time.Second, true, func(ctx context.Context) (bool, error) {
		...
		got, err = c.CoreV1().Namespaces().Create(ctx, namespaceObj, metav1.CreateOptions{})
		if err != nil {
			e2elog.Logf("Namespace %q creation failed: %v", err)
			return false, nil
		}
		return true, nil
	}); err != nil {
