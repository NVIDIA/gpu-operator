...
// operatorNamespace is the seed namespace object the admin-access label plumbing patches.
func operatorNamespace() *corev1.Namespace {
	return &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "test-operator"}}
}

func newTestDCGMState(t *testing.T) *configurableState {
	t.Helper()
	t.Setenv("DCGM_IMAGE", "nvcr.io/nvidia/cloud-native/dcgm:test")
	client := fake.NewClientBuilder().WithObjects(operatorNamespace()).Build()
