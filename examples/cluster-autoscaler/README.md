...
- Because of that prefix, you cannot use the Cluster Autoscaler's
  ...
  autoscalers differ in whether this can be configured — see step 3 of the
  walkthrough.
...
| `simulation/` | kind-based simulation of the full flow on a machine without GPUs — see [simulation/README.md](simulation/README.md) |
...
These steps target a real GPU cluster and are referenced from the
walkthrough below. To try the flow without GPUs first, see
...
## Walkthrough: real cluster with Cluster Autoscaler
...
- **A GPU resource the autoscaler cannot infer from the instance type.**
  ...
  walkthrough's test scales from zero with no extra setup. Per-profile MIG
