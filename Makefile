...
# Verify THIRD_PARTY_NOTICES.md is in sync with the dependency tree. Intended
# for CI on dependency changes.
.PHONY: notices-check
notices-check:
	@echo "- Checking if THIRD_PARTY_NOTICES.md is up to date..."
	@$(MAKE) notices
	@# git diff reports nothing for an untracked path, so an uncommitted
	@# notices file would otherwise pass this gate silently.
